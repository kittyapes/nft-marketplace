...
describe("Vesting", function () {
  ...
  beforeEach(async function () {
    ...
    await pixtToken.connect(alice).approve(vesting.address, totalSupply);
  });

  describe("check constructor", () => {
    it("check init value", async () => {
      expect(await vesting.connect(alice).pixtToken()).to.equal(
        pixtToken.address
      );
    });
    ...
    it("Revert if pixtToken is zero", async () => {
      const VestingFactory = await ethers.getContractFactory("Vesting");
      await expect(
        VestingFactory.deploy(constants.AddressZero)
      ).to.be.revertedWith("Vesting: token cannot be zero");
    });
  });
  ...
  describe("#initVesting function", () => {
    ...
    it("Revert if startTime is less than block time", async () => {
      ...
      ).to.revertedWith("Vesting: invalid startTime");
    });
    ...
    it("Revert if period is zero", async () => {
      ...
      ).to.revertedWith("Vesting: invalid period");
    });
    ...
    it("Revert if beneficiary is zero", async () => {
      ...
      ).to.revertedWith("Vesting: invalid beneficiary");
    });
  ...
  describe("#initVestings function", () => {
    ...
    it("Revert if amounts length is not same as startTimes length", async () => {
      ...
      ).to.revertedWith("Vesting: invalid length");
    });
    ...
    it("Revert if amounts length is not same as beneficiaries length", async () => {
      ...
      const startTime = currentTime.add(BigNumber.from("1000"));
      await expect(
        vesting.initVestings(amounts, [startTime, startTime], periods, [
          await bob.getAddress(),
        ])
      ).to.revertedWith("Vesting: invalid length");
    });
  ...
  describe("#claim function", () => {
    ...
    it("Revert if still locked", async () => {
      ...
      await expect(vesting.connect(bob).claim("0")).to.revertedWith(
    ...
    it("Should claim per week", async () => {
      ...
      const claimAmount = amount
        .mul(RELEASE_PERIOD.mul(BigNumber.from("2")))
        .div(period);
  ...
  describe("#claimInBatch function", () => {
    ...
    it("Revert if length is zero", async () => {
      await expect(vesting.claimInBatch([])).to.revertedWith(
        "Vesting: invalid length"
      );
    });
