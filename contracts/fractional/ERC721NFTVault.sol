...
contract ERC721TokenVault is ERC721EnumerableUpgradeable, ERC721HolderUpgradeable {
    ...
    /// @notice An event emitted when someone cashes in ERC721 tokens for ETH from an ERC721 token sale
    ...
    function updateAuctionLength(uint256 _length) external {
        require(msg.sender == curator, "update:not curator");
        require(
            _length >= IFractionalSettings(settings).minAuctionLength() &&
                _length <= IFractionalSettings(settings).maxAuctionLength(),
    ...
    function updateFee(uint256 _fee) external {
        ...
        require(
            _fee <= IFractionalSettings(settings).maxCuratorFee(),
    ...
    function _claimFees() internal {
        ...
        address govAddress = IFractionalSettings(settings).feeReceiver();
        uint256 govFee = IFractionalSettings(settings).governanceFee();
        ...
        if (curator != address(0)) {
            ...
            for (uint256 i = 0; i < curatorMint; i++) {
                _mint(curator, tot + i);
            }
        ...
        if (govAddress != address(0)) {
            uint256 tot = totalSupply();
            for (uint256 i = 0; i < govMint; i++) {
                _mint(govAddress, tot + i);
            }
    ...
    function updateUserPrice(uint256 _new) external {
        ...
        // previously they were not voting
            ...
            uint256 reservePriceMin = (averageReserve *
                IFractionalSettings(settings).minReserveFactor()) / 1000;
            require(_new >= reservePriceMin, "update:reserve price too low");
            uint256 reservePriceMax = (averageReserve *
                IFractionalSettings(settings).maxReserveFactor()) / 1000;
    ...
    /// @param _from the ERC721 token sender
    /// @param _to the ERC721 token receiver
    /// @param _tokenId the ERC721 token amount
        ...
        if (auctionState == State.inactive) {
            ...
            if (toPrice != fromPrice) {
                ...
                if (toPrice == 0) {
                    ...
                    votingTokens -= 1;
                    reserveTotal -= fromPrice;
                }
                // old holder is not a voter
                else if (fromPrice == 0) {
                    votingTokens += 1;
                    reserveTotal += toPrice;
                }
    ...
    function start() external payable {
        ...
        require(
            votingTokens * 1000 >=
                IFractionalSettings(settings).minVotePercentage() * totalSupply(),
